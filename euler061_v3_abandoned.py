...
""" 
v3
In this version, replacing generator function with a non-generator, in case the generator
...
def main():
    found_sum = 0
    g_lists = [g_3(), g_4(), g_5(), g_6(), g_7(), g_8()]
    for i in range(6):
        logging.debug(f"len of g{i+3}_list = {len(g_lists[i])}")
        logging.debug(f"g{i+3}_list = {g_lists[i]}")
    exit()
